...
import { PLAN_MATRIX, type PlanLimits } from "./planMatrix";
...
export class UsageLimitExceededError extends Error {
  code = BillingErrorCode.PLAN_LIMIT;
  status = 429;
  metric: UsageMetric;
  used: number;
  limit: number;
 ...
 */
function getPeriodStart(at: Date = new Date()): Date {
 ...
 */
function getMetricLimit(planName: PlanName, metric: UsageMetric): number | null {
...
export async function recordUsage(incr: UsageIncrement): Promise<void> {
  ...
  if (error) {
    ...
    if (existing) {
      const currentValue = existing[metricColumn as keyof typeof existing] as number;
 ...
 */
export async function getUsageSummary(workspaceId: string, at?: Date): Promise<UsageSummary> {
