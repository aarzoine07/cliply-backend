...
describe("render + thumbnail + publish pipelines", () => {
    ...
    it("renders clip, generates thumbnail, publishes youtube idempotently", async () => {
        ...
        expect(storage.upload).toHaveBeenCalledWith(BUCKET_RENDERS, `${WORKSPACE_ID}/${PROJECT_ID}/${CLIP_ID}.mp4`, expect.any(String), "video/mp4");
        expect(storage.upload).toHaveBeenCalledWith(BUCKET_THUMBS, `${WORKSPACE_ID}/${PROJECT_ID}/${CLIP_ID}.jpg`, expect.any(String), "image/jpeg");
...
function createStorageMock(initial) {
    ...
    return {
        ...
        list: vi.fn(async (bucket, prefix) => Array.from(files.keys())
    ...
    return {
        ...
        from(table) {
            if (table === "clips") {
                return {
                    select: () => ({
                        eq: (field, value) => Promise.resolve({ data: state.clips.filter((clip) => clip[field] === value), error: null }),
                    }),
                    update: (values) => ({
                        eq: (field, value) => {
            ...
            if (table === "projects") {
                return {
                    select: () => ({
                        eq: (field, value) => Promise.resolve({ data: state.projects.filter((project) => project[field] === value), error: null }),
                    }),
                    update: (values) => ({
                        eq: (field, value) => {
            ...
            if (table === "schedules") {
                return {
                    select: () => ({
                        eq: (field, value) => Promise.resolve({ data: state.schedules.filter((schedule) => schedule[field] === value), error: null }),
                    }),
                    update: (values) => ({
                        eq: (field, value) => {
