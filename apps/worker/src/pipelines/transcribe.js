...
export async function run(job, ctx) {
    try {
        ...
        await Promise.all([
            ...
            ensureTranscriptUploaded(ctx, workspaceId, payload.projectId, TRANSCRIPT_JSON, JSON.stringify(result.json ?? {}, null, 2), 'application/json', tempDir),
        ]);
...
async function resolveSourceKey(ctx, workspaceId, projectId, explicitExt) {
    if (explicitExt) {
...
async function ensureTranscriptUploaded(ctx, workspaceId, projectId, filename, contents, contentType, tempDir) {
...
export function pipelineTranscribeStub() {
    return 'transcribe';
}
