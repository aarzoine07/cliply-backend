...
function groupSegments(segments, minGapSec) {
...
function buildCandidate(segments, keywords) {
    ...
    const keywordHits = normalizedKeywords.reduce((total, keyword) => (lowerText.includes(keyword) ? total + 1 : total), 0);
...
async function fetchExistingClips(ctx, projectId) {
    const response = await ctx.supabase.from('clips').select('start_s,end_s').eq('project_id', projectId);
    const rows = response.data;
    return rows ?? [];
}
async function ensureProjectStatus(ctx, projectId, status) {
...
export function pipelineHighlightDetectStub() {
    return 'highlight';
}
