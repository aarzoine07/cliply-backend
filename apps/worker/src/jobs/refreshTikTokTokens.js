...
async function sealedBoxDecryptRef(ref) {
    if (!ref.startsWith("sbx:"))
        throw new Error("Invalid sealed-box reference");
    return Buffer.from(ref.slice(4), "base64url").toString("utf8");
}
async function sealedBoxEncryptRef(plaintext) {
    return `sbx:${Buffer.from(plaintext).toString("base64url")}`;
}
export async function refreshTikTokTokensJob() {
    ...
    for (const account of typedAccounts) {
        try {
            ...
            const { error: updateError } = await supabase
                ...
                .eq("platform", "tiktok");
            if (updateError) {
                console.error(`❌ Failed to update refreshed tokens for ${account.workspace_id}:`, updateError.message);
                continue;
            }
