...
export function buildRenderCommand(inputPath, outPath, opts) {
    ...
    const filterParts = [];
    filterParts.push(`[0:v]scale=${width}:${height}:force_original_aspect_ratio=cover,` +
    ...
    if (opts.makeThumb) {
        ...
        filterParts.push(`[thumb_src]trim=start=${thumbAt.toFixed(3)}:duration=0.05,setpts=PTS-STARTPTS[thumb]`);
        thumbLabel = '[thumb]';
    }
    ...
    args.push('-filter_complex', filtergraph, '-map', renderLabel, '-map', '0:a?', '-c:v', 'libx264', '-preset', preset, '-crf', String(crf), '-r', String(fps), '-movflags', '+faststart', '-c:a', 'aac', '-b:a', `${audioBitrate}k`, '-af', buildLoudnorm(loudnorm), outPath);
    if (thumbLabel && opts.makeThumb) {
        args.push('-map', thumbLabel, '-frames:v', '1', opts.makeThumb.outPath);
    }
    return { args, filtergraph };
}
function buildLoudnorm(values) {
    return `loudnorm=I=${values.I}:TP=${values.TP}:LRA=${values.LRA}`;
}
function escapeSubtitlesPath(path) {
