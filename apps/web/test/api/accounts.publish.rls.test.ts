...
import publishRoute from "../../src/pages/api/accounts/publish";
...
describe("T2 RLS: GET /api/accounts/publish (publish_config + connected_accounts)", () => {
  let jwtA = "";
  let jwtC = "";
  ...
  it("A (workspace member) sees connected accounts for workspace A", async () => {
    const res = await supertestHandler(toApiHandler(publishRoute), "get")
    ...
    expect(res.body).toHaveProperty("data.connectedAccounts");
    expect(Array.isArray(res.body.data.connectedAccounts)).toBe(true);
    expect(res.body.data.connectedAccounts.length).toBeGreaterThanOrEqual(1);
  });

  it("C (NOT a member) is blocked by RLS (connectedAccounts empty) when targeting workspace A", async () => {
    const res = await supertestHandler(toApiHandler(publishRoute), "get")
      .get("/?platform=youtube")
