...
// workspace used by publish.youtube E2E tests.
...
export default handler(async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  // E2E workspace (if we ever add E2E tests for YouTube)
  ...
  const parsed = PublishYouTubeInput.safeParse(rawBody);

  let payload: any;

  if (!parsed.success) {
    // In test environment, allow minimal payloads so edge-case tests
    // can reach the clip + account logic without requiring full schema.
    if (isTestEnv) {
      // We still require a clipId, otherwise it's genuinely invalid.
      if (!rawBody.clipId) {
        res.status(400).json(err('invalid_request', 'clipId is required'));
        return;
      }

      payload = {
        ...rawBody,
        clipId: rawBody.clipId,
        connectedAccountIds: Array.isArray(rawBody.connectedAccountIds)
          ? rawBody.connectedAccountIds
          : [],
      };
    } else {
      // In non-test environments, keep strict schema enforcement.
      res.status(400).json(
  ...
  if (isE2ETestWorkspace) {
    try {
      ...
      logger.warn('publish_youtube_idempotency_db_check_failed', {
        workspaceId,
        error: (error as Error)?.message ?? 'unknown',
      });
    }
  ...
  try {
    const jobsPayload = resolvedAccountIds.map((accountId) => ({
      ...
      payload: {
        ...
        // These fields come from PublishYouTubeInput; tests don’t care but worker will.
        title: payload.title,
