...
export default handler(async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  const idempotencyKey = keyFromRequest({ method: req.method, url: `/api/clips/${clipId}/approve`, body: parsed.data });

  const result = await withIdempotency(idempotencyKey, async () => {
    const clip = await admin.from('clips').select('id,status,workspace_id').eq('id', clipId).maybeSingle();
    if (clip.error) {
    ...
    if (!alreadyApproved) {
      ...
      if (update.error) {
        throw new HttpError(500, 'Failed to approve clip', 'clip_update_failed', update.error.message);
      }
      ...
      if (jobInsert.error) {
        throw new HttpError(500, 'Failed to enqueue render job', 'job_insert_failed', jobInsert.error.message);
      }
    }
