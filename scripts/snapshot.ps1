...
# 2) Tooling
$node = (& node -v) 2>&1; $pnpm = (& pnpm -v) 2>&1
$tsc = "tsc not found"
...
$migrateSqlPass = $false
# --- CORE TABLE MIGRATE PROBE ---
try {
  ...
  if ($LASTEXITCODE -eq 0 -and $nodeOut) {
    try { $tables = $nodeOut | ConvertFrom-Json } catch { $tables = @() }
    $core = @('workspaces','projects','clips','schedules','jobs','events','connected_accounts')
    $have = 0
    foreach ($t in $core) { if ($tables -contains $t) { $have++ } }
    if ($have -ge 7) { $migrateSqlPass = $true }
  }
} catch {}
$migrateFlag = "FAIL"
if ((($prismaS -join $newline) -match "Database schema is up to date|No pending migrations|Following migration") -or $migrateSqlPass) { $migrateFlag = "PASS" }
...
# 6) Dev server detect / start briefly in background if needed
$detected = "unknown"
...
# 7) Health (parse JSON; pass if ok:true and db != "error")
$healthFlag = "FAIL"
